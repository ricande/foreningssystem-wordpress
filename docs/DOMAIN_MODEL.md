...
- Creating an assignment requires continuous individual membership coverage for the whole assignment. Coverage is the merged intersection of member participations and periods. Adjacent inclusive intervals are continuous when the next one starts the day after the previous end, including when that coverage crosses memberships. An open assignment needs coverage that is still open. A gap rejects the assignment. An assignment cannot be created outside that coverage.
