...
| Membership participant | Custom table `assoc_membership_participant` | A single person id on the membership | Family and company contacts | role `member` or `contact`, inclusive start and end | Keep | Only `member` covers board eligibility, and only inside the participation dates. A later join is a new row. `assoc_membership_legacy` is the schema 14 copy and is not read by runtime screens |
