...
| Membership period | Custom table `assoc_membership` | Post meta on a person CPT, user meta | Many periods per person, non-overlapping dates, status | person_id, status, start/end | Keep after anonymization | Unique period identity. Do not encode history only as "current status" |
