...
A second file starts with `# foreningsplugin-members 2` and has `organization`, `membership`, `period` and `participant` rows. Participant rows also carry the participation start and end. A file from before those columns is still accepted: the start is the earliest period on that membership. It can add a later period to an existing number. It also excludes personal identity numbers. A protected identity export is not provided until the owner decides the policy.
