...
use Foreningssystem\Infrastructure\WordPress\WordpressMinutesLock;
...
$cleanup = static function () use ($wpdb, $audit): void {
    ...
    $wpdb->query($wpdb->prepare(
        ...
        'association_role',
        'grant_finalize_minutes',
        'revoke_finalize_minutes'
    ));
...
try {
    WordpressMinutesLock::update([RoleBundles::CHAIR, RoleBundles::SECRETARY]);
...
if (
    $denied !== true
    || user_can($secretary, Capabilities::FINALIZE_MINUTES) !== true
    || user_can($secretary, Capabilities::RECORD_MEETING) !== true
    || user_can($secretary, Capabilities::PUBLISH_MINUTES) !== false
    ...
    || user_can($boardMember, Capabilities::FINALIZE_MINUTES) !== false
    || ! str_contains($html, 'Ordföranden får låsa protokoll från början')
    ...
    || (string) $events[0]['action'] !== 'grant_finalize_minutes'
    || (int) $events[0]['actor_user_id'] !== 1
    || str_contains((string) $events[0]['action'], '@')
) {
    $fail('The minutes lock setting did not stay with the chosen role.');
}
...
if (user_can($secretary, Capabilities::FINALIZE_MINUTES) || ! user_can($chair, Capabilities::FINALIZE_MINUTES)) {
    \WP_CLI::error('The default minutes lock setting was not restored.');
}

\WP_CLI::success('The association can choose which role may lock minutes.');
